...
Route::get('/category/{id}', [CategoryController::class, 'show'])->middleware('auth'); //вывод списка инструментов по id категории
...
Route::get('/equipment/{id}', [EquipmentController::class, 'show'])->middleware('auth');//Вывод списка аренд с этим инструментом


Route::get('/user', [UserController::class, 'index'])->middleware('auth'); //Список пользователей
Route::get('/user/{id}', [UserController::class, 'show'])->middleware('auth'); //Заказ пользователя
